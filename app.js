...
window.setTimeout(() => {
  ...
  window.setTimeout(() => document.getElementById("introScreen")?.remove(), 900);
}, 1350);
...
const PROJECTS = [
  ...
  },
  {
    ...
    mainMedia: { label: "Final reel", type: "drive", id: "17M09AAN8D0vzx-_8YHT_WgctG3zGwwjj" },
...
document.querySelectorAll("a, button, .project-card, .interactive").forEach(bindInteractiveSurface);
...
function setStageHTML(html, externalUrl = "#") {
  stage.innerHTML = html;
  openExternal.href = externalUrl;
  openExternal.style.display = externalUrl === "#" ? "none" : "inline-flex";
}

function loadDriveMedia(id, label = "Drive media") {
  setStageHTML(`<iframe title="${label}" src="${drivePreview(id)}" loading="lazy" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>`, driveView(id));
}

function loadSketchfab(url) {
  setStageHTML(`<iframe title="Interactive 3D model" src="${url}" loading="lazy" allow="autoplay; fullscreen; xr-spatial-tracking" allowfullscreen></iframe>`, url.replace("/embed", ""));
}
...
function loadYouTubeVideo(video) {
  if (!video) return;
  setStageHTML(`<iframe title="${escapeHTML(video.title)}" src="${youtubeEmbed(video.id)}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; fullscreen" allowfullscreen></iframe>`, youtubeWatch(video.id));
...
function showMediaSection(section, index = 0) {
  ...
  mediaList.innerHTML = "";
  mediaList.classList.remove("youtube-list");
  ...
  if (section === "main") {
    ...
    else loadDriveMedia(media.id, media.label);
    return;
  }
...
function openProject(projectId) {
  ...
  document.getElementById("modalCategory").textContent = currentProject.categoryLabel;
  document.getElementById("modalTitle").innerHTML = `${currentProject.title}<br><span style="color:var(--orange)">${currentProject.subtitle}</span>`;
  ...
  document.getElementById("modalInfoGrid").innerHTML = `
    <div class="info-box"><small>Client</small><strong>${currentProject.client}</strong></div>
    <div class="info-box"><small>Year</small><strong>${currentProject.year}</strong></div>
    <div class="info-box"><small>Role</small><strong>${currentProject.role}</strong></div>
    <div class="info-box"><small>Tools</small><strong>${currentProject.tools}</strong></div>
  `;
  document.getElementById("modalHighlights").innerHTML = currentProject.highlights.map((item) => `<div class="highlight">${item}</div>`).join("");
  breakdown.innerHTML = `<h3>Full breakdown</h3><p>${currentProject.breakdown}</p>`;
...
function closeProject() {
  ...
  modal.querySelector(".modal-card")?.classList.remove("youtube-mode");
  clearYouTubeNowPlaying();
  stage.innerHTML = "";
  mediaList.innerHTML = "";
  currentProject = null;
...
window.addEventListener("keydown", (event) => { if (event.key === "Escape" && modal.classList.contains("open")) closeProject(); });
...
function buildEmailUrls() {
  ...
  return {
    gmail: `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent("drezeldeen.tantawi@gmail.com")}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`,
    outlook: `https://outlook.live.com/mail/0/deeplink/compose?to=${encodeURIComponent("drezeldeen.tantawi@gmail.com")}&subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`
  };
}
...
function sendProjectRequest(provider) {
  ...
  window.location.href = urls[provider] || urls.gmail;
}
...
function updateGameHud() {
  ...
  playerShip.style.setProperty("--shield-scale", `${.66 + shieldPct / 100 * .52}`);
  const powerParts = [];
...
function createEnemyShips() {
  const count = isMobileViewport() ? 14 : 18;
  for (let i = 0; i < count; i += 1) createEnemyShip(i);
  updateShipCounter();
}
...
function getLevelConfig(level) {
  ...
  const suffix = doubled ? " | Double hulls" : "";
  if (pattern === 1) return { pattern, ships: 7, bosses: 0, label: `Level ${level} | Scout ships${suffix}`, title: "Scout Wave", text: "Fast ships enter the arena. Stay mobile and clear the wave." };
  if (pattern === 2) return { pattern, ships: 0, bosses: 1, label: `Level ${level} | Boss duel${suffix}`, title: "Boss Duel", text: "Break the shield layers and dodge the return fire." };
  if (pattern === 3) return { pattern, ships: 14, bosses: 0, lasers: 3, label: `Level ${level} | Swarm pressure${suffix}`, title: "Swarm Pressure", text: "A dense formation rushes the player. Laser ships begin sustained fire." };
  if (pattern === 4) return { pattern, ships: 10, bosses: 1, lasers: 3, label: `Level ${level} | Boss swarm${suffix}`, title: "Boss Swarm", text: "Small ships guard the boss. Thin them out before committing." };
  return { pattern, ships: 8, bosses: 2, lasers: 2, label: `Level ${level} | Twin bosses${suffix}`, title: "Twin Bosses", text: "Two shielded bosses enter together. Watch the crossfire." };
}
...
function setupLevel(level) {
  ...
  showLevelTransition(`Chapter ${level} / ${FINAL_LEVEL}`, config.title, config.text, 1450);
...
function checkLevelComplete() {
  ...
  if (currentLevel >= FINAL_LEVEL) {
    ...
    showLevelTransition("Final chapter clear", "Mission Complete", "All ten levels cleared. Preparing victory screen.", 1200);
  ...
  showLevelTransition(`Level ${currentLevel} clear`, "Next Chapter", `Level ${currentLevel + 1} entering the arena.`, 1300);
  levelTransitionTimer = window.setTimeout(() => {
    currentLevel += 1;
    levelTransitionTimer = null;
    setupLevel(currentLevel);
...
function winGame() {
  ...
  showLevelTransition("Mission complete", "You Won", "You cleared level 10 and defeated the full arcade sequence.", 0, true);
...
function animateEnemyShips(now = performance.now()) {
  if (!playModeActive) {
    const ambientDelay = isMobileViewport() ? 180 : 110;
  ...
  const ambientLimit = playModeActive ? Infinity : particleBudget(10, 7);
  enemyShips.forEach((ship) => {
  ...
  maybeEnemyFire(now);
  queueNextShipFrame(playModeActive ? 0 : (lowPowerDevice ? 190 : 115));
}
...
function updateBossStatus() {
  ...
  bossStatus.textContent = active.map((boss) => `B${boss.index + 1} ${Math.max(0, Math.ceil(boss.hp))}/${Math.ceil(boss.maxHp)}`).join(" | ");
...
function spawnBoss(index = 0) {
  ...
  boss.active = true;
  boss.maxHp = Math.max(4, baseHp * BASE_BOSS_HP_MULTIPLIER * enemyHealthMultiplier);
  boss.hp = boss.maxHp;
  boss.x = window.innerWidth * (index % 2 ? .66 : .34) - 120;
...
function animateBoss(now, dt) {
  bossShips.forEach((boss) => {
    ...
    boss.x += boss.vx * dt;
    const bossWidth = Math.min(240, window.innerWidth * .44);
    if (boss.x < 24) {
      boss.x = 24;
...
function addPersistentCrack(el) {
...
function applyElementDamage(el, x, y, force) {
  ...
  if (level >= 2) addPersistentCrack(el);
  if (level >= 3) addPersistentCrack(el);
  const timer = window.setTimeout(() => clearElementDamage(el), 25000);
...
function addCrack(el) {
  ...
  crack.style.setProperty("--crack-x", `${(30 + Math.random() * 40).toFixed(1)}%`);
  crack.style.setProperty("--crack-y", `${(30 + Math.random() * 40).toFixed(1)}%`);
  el.appendChild(crack);
