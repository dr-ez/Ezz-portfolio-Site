...
async function runSmartIntroLoader() {
  ...
  const tasks = [
    ...INTRO_CRITICAL_ASSETS.map((src) => preloadIntroImage(src).finally(markComplete)),
    Promise.resolve(document.fonts?.ready).catch(() => {}).finally(markComplete),
    waitForTwoFrames().finally(markComplete)
  ];
  ...
  function progressLoop() {
    ...
    if (!finished) window.requestAnimationFrame(progressLoop);
  }
...
function bindInteractiveSurface(el) {
  ...
  el.addEventListener("mouseleave", () => {
    ...
    el.style.removeProperty("--weight-rx");
    el.style.removeProperty("--weight-ry");
    el.style.removeProperty("--weight-tx");
    el.style.removeProperty("--weight-ty");
  });
  ...
  el.addEventListener("pointermove", (event) => {
    ...
    window.requestAnimationFrame(() => {
      ...
      if (el.classList.contains("weight-reactive")) {
        el.style.setProperty("--weight-rx", `${(-weightY * 7).toFixed(2)}deg`);
        el.style.setProperty("--weight-ry", `${(weightX * 8).toFixed(2)}deg`);
        el.style.setProperty("--weight-tx", `${(weightX * 4).toFixed(2)}px`);
        el.style.setProperty("--weight-ty", `${(weightY * 4).toFixed(2)}px`);
      }
      localQueued = false;
    });
  });
