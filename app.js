...
// Reads a Google Drive video's true frame shape from its thumbnail. The
// thumbnail is generated at the video's real pixel dimensions, so reading
// naturalWidth/naturalHeight gives the exact aspect ratio with no API key and
// no CORS requirement (intrinsic size is exposed for cross-origin images).
function detectDriveAspect(id) {
  return new Promise((resolve) => {
    ...
    probe.src = driveThumbnail(id);
  });
}

// Sizes the stage to the active video's real aspect ratio while fitting both the
// available width and the viewport height, so the Drive player always has room
// to render its full control bar instead of collapsing into its mini layout.
function fitDriveStage() {
  ...
  const availH = Math.max(240, Math.round(window.innerHeight * 0.78));
...
function loadDriveMedia(id, label = "Drive media", options = {}) {
  ...
  const token = driveAspectToken;
  requestAnimationFrame(fitDriveStage);
  // Ask Drive for the real frame shape, then re-fit — corrects any video whose
  // hardcoded aspect is missing or wrong (e.g. a portrait reel left at 16/9).
  detectDriveAspect(id).then((realRatio) => {
    if (token !== driveAspectToken || !stageFit || !realRatio) return;
...
window.addEventListener("resize", () => { if (stageFit) fitDriveStage(); });
window.addEventListener("orientationchange", () => {
  if (stageFit) setTimeout(fitDriveStage, 250);
});
